...
public class ListCollectionsOutputBody {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(hasNextPage, pageNumber, pageSize, results);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ListCollectionsOutputBody.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in ListCollectionsOutputBody is not found in the empty JSON string", ListCollectionsOutputBody.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ListCollectionsOutputBody.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ListCollectionsOutputBody` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("results") != null && !jsonObj.get("results").isJsonNull()) {
        ...
        for (int i = 0; i < jsonArrayresults.size(); i++) {
          CollectionListItemBody.validateJsonElement(jsonArrayresults.get(i));
        }
      }
  }
