...
public class BatchRenameInputBody {
  ...
  private List<BatchRenameItem> functions;
   ...
   */
  @javax.annotation.Nullable
  public List<BatchRenameItem> getFunctions() {
    return functions;
  }

  public void setFunctions(@javax.annotation.Nullable List<BatchRenameItem> functions) {
    this.functions = functions;
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BatchRenameInputBody.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in BatchRenameInputBody is not found in the empty JSON string", BatchRenameInputBody.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BatchRenameInputBody.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `BatchRenameInputBody` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
      ...
      if (jsonObj.get("functions") != null && !jsonObj.get("functions").isJsonNull()) {
        ...
        for (int i = 0; i < jsonArrayfunctions.size(); i++) {
          BatchRenameItem.validateJsonElement(jsonArrayfunctions.get(i));
        }
      }
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       ...
       final TypeAdapter<BatchRenameInputBody> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(BatchRenameInputBody.class));

       return (TypeAdapter<T>) new TypeAdapter<BatchRenameInputBody>() {
           @Override
