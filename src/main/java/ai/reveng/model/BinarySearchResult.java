...
public class BinarySearchResult {
  ...
  @Override
  public int hashCode() {
    return Objects.hash(binaryId, binaryName, analysisId, sha256Hash, tags, createdAt, modelId, modelName, ownedBy);
  }
  ...
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!BinarySearchResult.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in BinarySearchResult is not found in the empty JSON string", BinarySearchResult.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!BinarySearchResult.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `BinarySearchResult` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }
